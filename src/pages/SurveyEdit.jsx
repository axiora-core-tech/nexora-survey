    ...
    <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 16, marginBottom: 40, flexWrap: 'nowrap' }}>
      <div style={{ minWidth: 0 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 16, fontFamily: 'Fraunces, serif', fontWeight: 300, fontSize: 12, color: 'rgba(22,15,8,0.35)' }}>
          ...
          <button onClick={copyLink} style={{ background: 'none', border: 'none', cursor: 'none', fontFamily: 'Fraunces, serif', fontSize: 12, color: 'rgba(22,15,8,0.35)', padding: 0, display: 'flex', alignItems: 'center', gap: 4, transition: 'color 0.2s' }} onMouseEnter={e => e.currentTarget.style.color = 'var(--coral)'} onMouseLeave={e => e.currentTarget.style.color = 'rgba(22,15,8,0.35)'}>
            ⎘ Copy link
          </button>
        </div>
      </div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0, flexWrap: 'nowrap' }}>
        {sv.status !== 'active' && <button onClick={() => chg('active')} style={{ padding: '9px 16px', borderRadius: 999, border: 'none', background: 'rgba(30,122,74,0.12)', color: 'var(--sage)', fontFamily: 'Syne, sans-serif', fontWeight: 700, fontSize: 10, letterSpacing: '0.1em', textTransform: 'uppercase', cursor: 'none', whiteSpace: 'nowrap', transition: 'all 0.2s' }} onMouseEnter={e => { e.currentTarget.style.background = 'var(--sage)'; e.currentTarget.style.color = '#fff'; }} onMouseLeave={e => { e.currentTarget.style.background = 'rgba(30,122,74,0.12)'; e.currentTarget.style.color = 'var(--sage)'; }}>▶ Activate</button>}
        {sv.status === 'active' && <button onClick={() => chg('paused')} style={{ padding: '9px 16px', borderRadius: 999, border: 'none', background: 'rgba(255,184,0,0.12)', color: '#A07000', fontFamily: 'Syne, sans-serif', fontWeight: 700, fontSize: 10, letterSpacing: '0.1em', textTransform: 'uppercase', cursor: 'none', whiteSpace: 'nowrap', transition: 'all 0.2s' }} onMouseEnter={e => { e.currentTarget.style.background = 'var(--saffron)'; e.currentTarget.style.color = 'var(--espresso)'; }} onMouseLeave={e => { e.currentTarget.style.background = 'rgba(255,184,0,0.12)'; e.currentTarget.style.color = '#A07000'; }}>⏸ Pause</button>}
        {['expired','closed'].includes(sv.status) && hasPermission(profile?.role,'resume_survey') && <button onClick={() => chg('active')} style={{ padding: '9px 16px', borderRadius: 999, border: 'none', background: 'rgba(30,122,74,0.12)', color: 'var(--sage)', fontFamily: 'Syne, sans-serif', fontWeight: 700, fontSize: 10, letterSpacing: '0.1em', textTransform: 'uppercase', cursor: 'none', whiteSpace: 'nowrap' }}>↺ Resume</button>}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
          ...
          <div>
            ...
            <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
              <input type="color" value={sv.theme_color || '#FF4500'} onChange={e => s('theme_color', e.target.value)} style={{ width: 48, height: 48, borderRadius: 12, border: '1px solid rgba(22,15,8,0.1)', cursor: 'none', padding: 2 }} />
              ...
              <div style={{ display: 'flex', gap: 4 }}>
                {[[-1,'↑'],[1,'↓']].map(([d, sym]) => (
                  <button key={d} onClick={() => moveQ(q._id, d)} disabled={(d===-1&&i===0)||(d===1&&i===qs.length-1)}
                    style={{ width: 30, height: 30, borderRadius: 8, border: 'none', background: 'none', cursor: 'none', color: 'rgba(22,15,8,0.3)', fontSize: 13, display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'all 0.15s', opacity: ((d===-1&&i===0)||(d===1&&i===qs.length-1)) ? 0.2 : 1 }}
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              ...
              <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
                ...
                <label style={{ display: 'flex', alignItems: 'center', gap: 8, fontFamily: 'Syne, sans-serif', fontSize: 10, fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', color: 'rgba(22,15,8,0.4)', cursor: 'none', whiteSpace: 'nowrap', flexShrink: 0 }}>
                  <input type="checkbox" checked={q.is_required} onChange={e => sQ(q._id, 'is_required', e.target.checked)} style={{ width: 16, height: 16, accentColor: 'var(--coral)', cursor: 'none' }} />
                  Required
                </label>
              </div>
                ...
                <div style={{ marginTop: 4, paddingLeft: 16, borderLeft: '2px solid rgba(255,69,0,0.2)', display: 'flex', flexDirection: 'column', gap: 8 }}>
                  {(q.options || []).map((o, j) => (
                    <div key={j} style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                      ...
                      <button onClick={() => delOpt(q._id, j)} style={{ background: 'none', border: 'none', cursor: 'none', color: 'rgba(22,15,8,0.2)', fontSize: 14, padding: 4, transition: 'color 0.15s', lineHeight: 1 }}
      ...
      <div style={{ position:'fixed',inset:0,background:'rgba(22,15,8,0.3)',backdropFilter:'blur(8px)',zIndex:50,display:'flex',alignItems:'center',justifyContent:'center',padding:20 }} onClick={() => setShareOpen(false)}>
        <div style={{ background:'var(--warm-white)',borderRadius:24,padding:36,width:'100%',maxWidth:400,boxShadow:'0 40px 100px rgba(22,15,8,0.2)' }} onClick={e => e.stopPropagation()}>
          <div style={{ display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:24 }}>
            ...
            <button onClick={() => setShareOpen(false)} style={{ background:'none',border:'none',cursor:'none',color:'rgba(22,15,8,0.3)',fontSize:18,lineHeight:1,transition:'color 0.2s' }} onMouseEnter={e=>e.currentTarget.style.color='var(--espresso)'} onMouseLeave={e=>e.currentTarget.style.color='rgba(22,15,8,0.3)'}>✕</button>
          </div>
              ...
              return (
                <div key={u.id} style={{ display:'flex',alignItems:'center',justifyContent:'space-between',padding:'12px 16px',background:'var(--cream)',borderRadius:14 }}>
                  ...
                  {shared
                    ...
                    : <button onClick={() => share(u.id)} style={{ fontFamily:'Syne,sans-serif',fontSize:10,fontWeight:700,letterSpacing:'0.1em',textTransform:'uppercase',color:'var(--espresso)',background:'none',border:'none',cursor:'none',transition:'color 0.2s' }} onMouseEnter={e=>e.currentTarget.style.color='var(--coral)'} onMouseLeave={e=>e.currentTarget.style.color='var(--espresso)'}>Share</button>}
                </div>
              );
            })}
          </div>
