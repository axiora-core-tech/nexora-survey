    ...
    <div className="animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-surface-500 mt-1">Manage and track all your surveys</p>
        </div>
          ...
          {['all', 'active', 'draft', 'paused', 'expired', 'closed'].map((s) => (
            <button
              ...
              className={`px-3 py-2 rounded-xl text-sm font-medium transition-all ${
                statusFilter === s
                  ? 'bg-nexora-50 text-nexora-700 border border-nexora-200'
      ...
      {loading ? (
        <div className="space-y-3">
          {[1, 2, 3, 4].map((i) => (
            <div key={i} className="h-20 bg-white rounded-2xl animate-pulse border border-surface-100" />
          ))}
        </div>
      ) : filtered.length === 0 ? (
        <div className="text-center py-20">
          <HiOutlineClipboardList className="w-16 h-16 text-surface-300 mx-auto mb-4" />
        ...
        <div className="space-y-3">
          {filtered.map((survey) => (
            <div key={survey.id} className="glass-card-hover p-4 md:p-5 flex items-center gap-4">
              {/* Color indicator */}
              <div
                className="w-2 h-12 rounded-full flex-shrink-0"
                style={{ backgroundColor: survey.theme_color || '#6366f1' }}
              />
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1">
                  <Link
                    ...
                    className="text-sm md:text-base font-semibold text-surface-800 hover:text-nexora-700 truncate transition-colors"
                  >
                    {survey.title}
                  </Link>
              ...
              <div className="flex items-center gap-2 flex-shrink-0">
                <button
                  ...
                  className="btn-ghost p-2 text-surface-400 hover:text-nexora-600"
                  title="Copy link"
                >
                ...
                <div className="relative">
                  <button
                    ...
                    className="btn-ghost p-2 text-surface-400"
                  >
                  ...
                  {openMenu === survey.id && (
                    <>
                      ...
                      <div className="absolute right-0 top-10 z-20 w-48 bg-white rounded-xl shadow-glass-lg border border-surface-200 py-1.5 animate-scale-in">
                        <button
                        ...
                        {(survey.status === 'expired' || survey.status === 'closed') &&
                          ...
                          <button
                            onClick={() => handleStatusChange(survey.id, 'active')}
                            className="w-full flex items-center gap-2 px-4 py-2 text-sm text-nexora-600 hover:bg-nexora-50"
                          >
